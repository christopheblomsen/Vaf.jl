...
"""
function humlicek!(a::CuDeviceVector{Float32, 1},
                v::CuDeviceVector{Float32, 1},
                ans::CuDeviceVector{ComplexF32, 1})
    ix = (blockIdx().x - 1) * blockDim().x + threadIdx().x

    w = Complex(0f0, 0f0)
    if (ix > 1 && ix < size(a, 1) && ix < size(v, 1))
        z = CUDA.Complex{Float32}(a[ix], v[ix])
...
function humlicek!(a::T,
                v::T, 
                ans::T) where T <: AbstractFloat
    w = CUDA.Complex(0f0, 0f0)
    z = CUDA.Complex{Float32}(a, v)
    s = CUDA.abs(CUDA.real(a)) + CUDA.imag(v)
    if s > 15.0f0
        # region I
        w = im * invSqrtPi * z / (z * z - 0.5f0)
    elseif s > 5.5f0
        ...
        x, y = CUDA.real(z), CUDA.imag(z)
        t = y - im * x
        if y >= 0.195f0 * CUDA.abs(x) - 0.176f0
            # region III
            w = ((16.4955f0 + t * (20.20933f0 + t * (11.96482f0 + t * (3.778987f0 + 0.5642236f0 * t))))
               / (16.4955f0 + t * (38.82363f0 + t * (39.27121f0 + t * (21.69274f0 + t * (6.699398f0 + t))))))
        else
...
function voigt_humlicek!(ans, a, v)
    ix = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if (ix <= size(ans,1))
    z = v[ix] + a[ix] * im
    s = CUDA.abs(CUDA.real(z)) + CUDA.imag(z)
    if s > 15.0f0
        # region I
        w = im * invSqrtPi * z / (z * z - 0.5f0)
    elseif s > 5.5f0
